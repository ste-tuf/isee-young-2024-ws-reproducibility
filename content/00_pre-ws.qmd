...
-   **Mac:** Download the installer from the [Git
    ...
    install git-gui using homebrew: `brew install git-gui`
...
-   **Windows, Mac, Linux:** All the packages can be installed with R
    ...
    ``` R
    install.packages("quarto", "tidyverse", "gt", "targets")
    ```
...
Data used in the cohort is from NHANES study. You can either download it here: 

or run the following lines in R: 
...
You can get the workshop scripts and documents by cloning the GitHub
repo:

``` bash
git clone https://github.com/ste-tuf/isee-young-ws-reproducibility.git
```
